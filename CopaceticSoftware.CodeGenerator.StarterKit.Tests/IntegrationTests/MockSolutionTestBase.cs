...
namespace CopaceticSoftware.CodeGenerator.StarterKit.Tests.IntegrationTests
{
    public abstract class MockSolutionTestBase : IntegrationTestBase
    {
        ...
        protected virtual IMicrosoftBuildProjectLoader BuildMockMicrosoftBuildProjectLoader()
        {
            ...
            loader.Stub(
                x =>
                    x.AddAutogeneratedClassFile(Arg<string>.Is.Anything, Arg<string>.Is.Anything,
                        Arg<string>.Is.Anything))
                .Do(
                    (Action<string, string, string>)
                    ((projectFileName, classFileName, dependentClassFile) =>
                     {
                         var project = _MockSolution.Projects.FirstOrDefault(
                             p => p.FileName.Equals(
                                 projectFileName, StringComparison.InvariantCultureIgnoreCase));

                         if (null == project)
                             throw new Exception("Couldn't find Project [" + projectFileName +"] in MockSolution.");

                         string fileContents;
                         if (_newFileWriteQueue.TryRemove(classFileName, out fileContents))
                         ...
                         {
                             throw new Exception("Couldn't find file contents for [" + classFileName + "]");
                         }
                     })
                );
